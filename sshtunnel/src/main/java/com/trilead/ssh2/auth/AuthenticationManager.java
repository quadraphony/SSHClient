...
public class AuthenticationManager implements MessageHandler
{
	...
	public void setConnectionMonitors(Vector<ConnectionMonitor> connMonitors) {
		this.connMonitors = (Vector<ConnectionMonitor>) connMonitors.clone();
	}
	...
	byte[] getNextMessage() throws IOException
	{
		while (true)
		{
			...
			String banner = sb.getBanner();
			
			if (banner != null) {
				for (ConnectionMonitor listener : connMonitors) {
					listener.onReceiveInfo(ConnectionMonitor.SERVER_BANNER, banner);
				}
			}
		}
	}
