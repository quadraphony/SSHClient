...
public class Connection
{
	...
	public ConnectionInfo connect(ServerHostKeyVerifier verifier, int connectTimeout, int readTimeout, int kexTimeout)
		...
		if (!this.compression) {
			String[] strArr = new String[]{"none"};
			this.cryptoWishList.c2s_comp_algos = strArr;
			strArr = new String[]{"none"};
			this.cryptoWishList.s2c_comp_algos = strArr;
		}
	...
	public synchronized void setCompression(boolean z) throws IOException {
		if (this.tm != null) {
			throw new IOException(new StringBuffer().append(new StringBuffer().append("Connection to ").append(this.hostname).toString()).append(" is already in connected state!").toString());
		}
		this.compression = z;
	}
	...
	public synchronized long ping() throws IOException
	{
		...
		return cm.requestGlobalTrileadPing();
	}
