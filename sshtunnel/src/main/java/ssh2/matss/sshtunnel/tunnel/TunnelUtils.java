		...
		if (!payload.isEmpty()) {
			...
			while (matcher.find()) {
				...
				if (!listKeys.contains(String.format("[%s]", group)) && !group.startsWith("rotate=")) {
					VpnStatus.logWarning(String.format("<strong>Warning:</strong> [%s] não suportado", group));
				}
			}*/
			...
			//VpnStatus.logInfo("Payload: " + payload.replace("\n", "\\n").replace("\r", "\\r"));
		}

		//VpnStatus.logInfo("Host Address: " + hostname);
		//VpnStatus.logInfo("Server Port: " + port);
		return payload;
	}
